...
patch() {
	...
	if [ -f "./download/$1.apk" ]; then
		...
		# Extract app version (normalize 19-44-39 → 19.44.39)
		app_version=$(echo "$version" | sed 's/-/./g')
		# Extract patch version from patches file (revanced-patches-v5.1.1.rvp → 5.1.1)
		patch_version=$(ls revanced-patches-*.rvp 2>/dev/null | sed -E 's/.*-v([0-9]+\.[0-9]+\.[0-9]+)\.rvp/\1/')
		# Flavor (manual)
		flavor="revanced-extended"
		# Base name (remove -stable etc.)
		app_name="youtube"
		# Architecture detection
		case "$1" in
		*arm64*) arch="arm64-v8a" ;;
		*armeabi*) arch="armeabi-v7a" ;;
		*x86_64*) arch="x86_64" ;;
		*x86*) arch="x86" ;;
		*) arch="universal" ;;
		esac
		# Final output filename
		outfile="./release/${app_name}_${arch}_${flavor}_v${app_version}_patch-v${patch_version}.apk"
		...
		# Run CLI with new filename
		eval java -jar *cli*.jar $p$b $m$opt \
			--out="$outfile" \
			--keystore=./src/$ks.keystore $pu$force $a./download/$1.apk
...
split_arch() {
	...
	if [ -f "./download/$1.apk" ]; then
		...
		eval java -jar revanced-cli*.jar patch \
			...
			--out=./release/$1-${archs[i]}-$2.apk ./download/$1.apk
	else
		red_log "[-] Not found $1.apk"
		exit 1
	fi
